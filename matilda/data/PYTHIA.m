...
[Znorm,out.mu,out.sigma] = zscore(Z);  % normalised of z
display("mean", out.mu)
display("sigma", out.sigma)
...
disp('-------------------------------------------------------------------------');
% 2. Choose Kernel and SVM library
...
%   train svm model either using LIBSVM @ Matlab's
%   calculate confusion matrics to evaluate model performance
...
for i=1:nalgos
    ...
    rng(state);
    % Validate: use confusion metrics to evaluate model performance
...
% 4. Evaluation: (precision, recall, accuracy)
if nalgos>1
...
% B. Set hyperparameters if not defined
if any(isnan(params))
    rng('default');
