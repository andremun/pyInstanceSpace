...
class SiftedStage(Stage[SiftedInput, SiftedOutput]):
    class SiftedStage(Stage[SiftedInput, SiftedOutput]):
        """A stage in the processing pipeline for feature selection and instance 
        filtering.

        This class is responsible for executing the SIFTED stage, which involves 
        selecting relevant features based on correlation with performance metrics and 
        applying clustering techniques for further refinement. If the filtering flag is 
        enabled, the stage also filters instances based on specific criteria.
        ...
        _compute_correlation(x: NDArray[np.double], y: NDArray[np.double]) -> 
            ...
            Calculates the Pearson correlation coefficients between features and performance metrics.
        
        evaluate_cluster(x_aux: NDArray[np.double], rng: np.random.Generator) -> 
            tuple[list[float], NDArray[np.intc]]:
            Evaluates clusters based on silhouette scores for different cluster configurations.
    ...
    def sifted(
        ...
        """Execute the core Sifted stage functionality to process feature selection and performance.
        ...
        -------
        SiftedOutput
            Output of the Sifted stage, including selected feature and algorithm indices, 
            correlation coefficients, performance metrics, silhouette scores, and feature 
            selection flags.
    ...
    def _sifted(
        ...
        """Executes feature selection and optional filtering for instance selection.
