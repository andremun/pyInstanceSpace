...
csv_path_num_good_algos_input = script_dir / "test_data/prelim/input/filter/input_numGoodAlgos.csv"
...
num_good_algos_input = pd.read_csv(csv_path_num_good_algos_input, header=None).to_numpy()
...
subset_index_output_fractional = pd.read_csv(script_dir / "test_data/prelim/output/filter/output_subsetIndex_fractional.csv", header=None).to_numpy().flatten()
...
def test_filter_post_prelim_fractional() -> None:
    ...
    (subset_index, x, y, x_raw, y_raw, y_bin, beta, num_good_algos, y_best, p) = prelim._filter_post_prelim(
        x_input, y_input, y_bin_input, y_best_input, x_raw_input, y_raw_input, p_input, num_good_algos_input, beta_input,
        selvars.small_scale_flag, selvars.small_scale, selvars.file_idx_flag, selvars.file_idx, selvars.feats, selvars.algos,
        selvars.selvars_type, selvars.min_distance, selvars.density_flag
    )
