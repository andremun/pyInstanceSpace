...
def test_general_options_default_matches_previously_hardcoded_behaviour():
    general = GeneralOptions.default()
    assert general.verbose is True
    assert general.seed == 0


def test_instance_space_options_default_general_is_backward_compatible():
...
def test_instance_space_options_accepts_explicit_general():
    custom = GeneralOptions.default(verbose=False, seed=42)
    options = InstanceSpaceOptions.default(*([None] * 12), general=custom)
    assert options.general is custom


def test_from_dict_defaults_general_when_absent():
    options = InstanceSpaceOptions.from_dict({})
    assert options.general.verbose is True
    assert options.general.seed == 0


def test_from_dict_parses_general_section():
...
def test_from_dict_allows_null_seed_for_nondeterministic_runs():
