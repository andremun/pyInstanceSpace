...
% -------------- 1. Collect Data (from metadata.csv and options.json) --------------
...
% ------------------ 2. Processing Options (reads and displays options specified in the options.json file)  ----------------------------------
...
% ------------------------------ 3. Parallel Processing Setup (IDK??? ) ----------------------------------
...
% ---------------------- 4. Loading Data (load data from csv into matlab variable)  ------------------------
...
% ----------- 5. Feature and Algorithm Selection: provides options to select specific features and algorithms to work with based on the options specified  ------------------
...
% ---------------- 6. Data Preprocessing (handling missing values, scaling and normalisation) --------------------
...
% ------------- 7. Feature Selection (performs automated feature selection using a method called SIFTED if enabled) ------
...
if opts.sifted.flag
    ...
    disp('=========================================================================');
    % Output modified model.data.X and the selected features, model.sifted
    [model.data.X, model.sifted] = SIFTED(model.data.X, model.data.Y, model.data.Ybin, opts.sifted);
    % Updates 'model.data.X' and 'model.data.featlabels' based on selected variables stored in 'model.sifted.selfvars'
    model.data.featlabels = model.data.featlabels(model.sifted.selvars);
    model.featsel.idx = model.featsel.idx(model.sifted.selvars);

    % If bydensity is true, proceeds to create small scale experiment for validation (bydensity is a boolean var i think set in json file if small scale experiment is needed)
    if bydensity
        disp('-> Creating a small scale experiment for validation based on density.');
...
% ------ 8. Project analysis (perform 2 D projection using PILOT), Empirical Bounds Calculations (calculate empirical bounds using CLOISTER)
...
% --------- 9. Algorithm Selection : train model to seperate space into classes of good and bad perfromance ----------
...
% ----------- 10. Footprint Analysis: footprint analysis using trace algorithm, either using simulated data from Pythia or experimental data  ------------- 
...
% ------------- 11. Output Generation: saving results as MATLAB files, csv and creating plots ------------- 
disp('-------------------------------------------------------------------------');
