...
%
% -------------------------------------------------------------------------
% Check number of available workers for parallel processing
...
% ---------------------------------------------------------------------
% 1st selection: Features are selected based on their correlation with performance. Features with a correlation coefficient greater than or equal to a threshold (opts.rho) are selected.
...
% ---------------------------------------------------------------------
% After 1st selection, if #features less than 3, skip second selection, clustering
nfeats = size(Xaux,2);
if nfeats<=1
    error('-> There is only 1 feature. Stopping space construction.');
elseif nfeats<=3
...
% ---------------------------------------------------------------------
% 2nd selection: Feature selection based on correlation clustering is performed using the k-means algorithm. The silhouette values for different numbers of clusters (opts.K) are evaluated.
...
% ---------------------------------------------------------------------
% Features are clustered using k-means
rng('default');
...
% ---------------------------------------------------------------------
% All possible combinations of features are generated, taking one feature from each cluster.
...
% Choose optimal combinations: 
    % 1. if combinations > 1000 -> use geneetic algorithm
    % 2. if combination = 1 -> that's the optimal feature combination
    % 3. if combinations < 1000, loop through all features, compute out-of-bag eror using 'cosfcn' function. After looping, get a sorted combination based on minimum sum of OOB error
...
% =========================================================================
% costfcn used to calculate OOB error (<1000 combinations)
...
% =========================================================================
% fcnforga function used for GA algorithm (>1000 combinations)
