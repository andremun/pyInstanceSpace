...
| F17 | — (Python-only correctness gap, surfaced by F8's audit) | Fix `InstanceSpace._explore_trace()`'s use of `.covers()` (boundary-inclusive) where training's `Footprint.from_polygon()`/`contra()` use `.contains()` (boundary-exclusive) — a real inconsistency between build and explore membership tests for the same footprint polygons. Originally filed as #315 during F8 (v1.66) with the fix direction initially reversed (assumed `.contains()` was the bug); corrected this session after reading MATLAB's `TRACE_legacy.m` and `TRACErescore` directly — both use `isinterior` (boundary-exclusive, matching Shapely's `.contains()`), not `inpolygon` (boundary-inclusive) — so the actual fix is `_explore_trace()`'s `.covers()` → `.contains()`, not the training side. | Filed as its own scoped issue (#315, retitled "F17 — Fix `_explore_trace()`'s `.covers()` to `.contains()`, matching MATLAB's `isinterior`/`TRACErescore`"), linked as a sub-issue of #260. Not implemented — filed per direct instruction ("file the TRACE contains/covers fix as its own scoped task"). | **[Behavior-changing]** — changes which instances an explore-time query counts as "in good"/"in best" for points exactly on a footprint's boundary; needs its own before/after verification against the existing TRACE reference tests once implemented, not assumed safe from the diff's small size. |
