...
> **Historical roadmap; current-status banner (2026-09-04).** Status rows and
> “not started” claims below are preserved as dated history. Current parity uses the
> installed 423-file, `matlab-verified` R2026a Update 4 v2 oracle from MATLAB v0.9.1
> at `98a01ac` (86 provenance tests; 41 current readers). Issue #316 is implemented
> through the typed stage-owned
...
| 11 | F16 | **Added v1.64.** Soft preference after F8 (its `TRACErescore` piece is F8's own explore/build-unification problem, avoid building it twice); 2D phase independent of F2, 3D phase blocked on it. Highest-risk item in this table — no MATLAB source exists for `alphaShape`'s internals to port, so bit-for-bit parity may not be achievable at all (see companion pathways doc). |
...
**F17 correction (v1.70):** the F17 row and v1.66/v1.68 history below record the old filing
state. Official MATLAB documentation confirms `polyshape.isinterior` includes boundary points.
Issue #315's proposed `.covers()` to `.contains()` change is invalid; `.covers()` is retained and
the active pathway is corrected in `docs/python_implementation_pathways.md`.
...
| F8 | — | Unify `explore()` with build-time stage code (predict-mode dispatch on `PythiaStage`/`TraceStage`, matching MATLAB calling the same `PYTHIA()`/`TRACE()` in both modes) | **Implemented and verified (v1.66).** PYTHIA half (the real, confirmed duplication): `PythiaStage._determine_selections`'s weighting formula split into a new shared `_weighted_selection(nalgos, precision, y_hat)` static method - `_determine_selections` (training) and `_explore_pythia` (explore) both call it now, instead of each hand-rolling the same `weighted = y_hat * precision; best = max; argmax` arithmetic; `_explore_pythia` also now calls `PythiaStage._compute_znorm()` for its mu/sigma instead of recomputing the identical formula inline. TRACE half: audited directly (`TraceStage`'s `build()`/`contra()` vs `_explore_trace()`) and found **no live duplication to extract** - `_explore_trace()` already correctly reuses the trained `Footprint.polygon` rather than re-deriving footprint math, so F8's "shared function" goal doesn't apply the way it did for PYTHIA. The audit did surface a related, separate, previously-undiscovered inconsistency (training's `Footprint.from_polygon`/`contra()` use `.contains()`, boundary-exclusive; explore's `_explore_trace` uses `.covers()`, boundary-inclusive, matching MATLAB's `inpolygon`) - filed as its own issue (#315), not fixed here since it would change existing training-time `density`/`purity` output. Drift-detection test added (`test_pythia_weighted_selection_is_shared_by_build_and_explore`, `tests/test_explore_pythia.py`): breaks `_weighted_selection`, confirms both the training-path call and `_explore_pythia` immediately reflect the broken result. The `nalgos == 1` explore-path case (previously untested, flagged as the likeliest divergence spot) is now tested (`test_pythia_selection0_nalgos_equals_one`) and found to reproduce a **separate, pre-existing bug** (selection index `1` instead of `0` for a single-algorithm portfolio) - filed as #314, not fixed here (see that issue for why: it's a real `[Behavior-changing]` fix in its own right, not part of F8's de-duplication scope). Full suite passes (see v1.66 below for the run). | **[Behavior-changing risk], verified additive in practice** — refactored existing code (both halves), but every shared-function extraction preserves the exact prior formula bit-for-bit (confirmed via the full `tests/matlab_reference/` suite passing unchanged, plus the new drift-detection test proving the two call sites can no longer silently diverge); no tolerance thresholds were touched. |
| F9 | — | Expand `explore()` to full evaluation scope: algorithm reconciliation + ground-truth performance metrics, matching MATLAB's `evaluateTestSet` | **Implemented and verified (v1.66).** `compute_binary_performance(y_raw, perf_opts, general_opts, log_prefix)` extracted from `PrelimStage._prelim()` into a standalone function in `prelim.py` (also removes `_select_best_algorithms` as a separate method - its tie-breaking logic folded in); `PrelimStage._prelim()` and `explore()`'s new evaluation path both call it, the latter with `log_prefix="EXPLORE"` (matching MATLAB's own "[EXPLORE] Calculating the binary measure of performance" line, satisfying the "make the silent branch visible" pathway step for free). New `InstanceSpace._build_test_algo_matrix()` reindexes a test set's `algo_*` columns to the trained algorithm order, case-insensitively (mirrors `strcmpi`); an algorithm present in training but absent from the test set becomes an all-NaN column (`compute_binary_performance`'s existing NaN handling excludes it from ever being "best", matching MATLAB's missing-ground-truth convention) and reports `NaN` accuracy/precision/recall/confusion-matrix rather than one computed against a fabricated label; a "new" algorithm in the test set absent from training was initially dropped (deferred per the original decision) - **superseded in v1.67: implemented at full MATLAB parity**, see below. New `InstanceSpace._explore_evaluate()` computes real per-algorithm `accuracy`/`precision`/`recall`/confusion-matrix (`sklearn.metrics`, matching MATLAB's `tp/(tp+fp)`/`tp/(tp+fn)`/`(tp+tn)/ninst` formulas) from PYTHIA's already-computed `y_hat` (not recomputed) against the newly-available ground truth - this is the corrected scope (real metrics, not just ground-truth label exposure) confirmed against MATLAB's `PYTHIAevalMode` in the prior pass. New `ExploreStage.EVALUATION` enum member, yielded by `explore_stage_iter()` only when `len(test_metadata.algorithm_names) > 0`; `ExploreResult` extended with 8 new `\| None` fields (`y_actual`, `y_best_actual`, `p_actual`, `beta_actual`, `accuracy_actual`, `precision_actual`, `recall_actual`, `cvcmat_actual`), all `None` in the feature-only case. 15 new tests across `tests/test_explore_evaluate.py` (numerics: case-insensitive reindexing, missing-algorithm NaN handling, new-algorithm dropping, metric computation cross-checked against an independently-computed `sklearn.metrics.confusion_matrix`) and `tests/test_explore_stage_iter.py` (orchestration: `EVALUATION` yielded/omitted correctly, existing no-ground-truth tests extended to assert all 8 new fields stay `None`). **v1.67: the "new algorithm absent from training" edge case, implemented at full MATLAB parity** (user's explicit choice over an evaluation-metrics-only scope) - `_explore_pythia()`/`_explore_trace()` gained an `n_new_algos` parameter widening `y_hat`/`pr0_hat`/`in_good`/`in_best` with `False`/`0.0` placeholders (matching MATLAB's `PYTHIAevalMode`/`TRACEthrow3` padding, not just NaN-padding the metrics), `_build_test_algo_matrix()` now appends real performance columns for new algorithms instead of dropping them, and a new `ExploreResult.algo_labels` field exposes the resulting widened algorithm order. New algorithms participate as full candidates in `y_best_actual`/`p_actual`/`beta_actual` but report `NaN` accuracy/precision/recall/cvcmat (no trained classifier exists to score them). | **[Additive], verified.** New fields default to `None`; the existing no-ground-truth test path (`test_explore_maps_stage_outputs_onto_the_result`) now explicitly asserts all 8 new fields are `None`, locking in "existing feature-only callers see no change" rather than assuming it. The v1.67 new-algorithm widening only activates when the test set actually introduces an unseen algorithm - the common case (no new algorithms) is unaffected, verified via the pre-existing test suite passing unchanged. |
...
| F16 | Phase 6 (TRACE) | Port TRACE3, MATLAB's current default footprint algorithm (`core/TRACE.m`'s `method='trace3'` branch) — Python's `trace.py` only implements the `legacy` DBSCAN-based algorithm (F11's corrected finding), and Python's own `TraceOptions.method` still defaults to `"legacy"`, the opposite of MATLAB's current default. `method='trace3'` currently raises `NotImplementedError` (deliberate stub, not silent). Added v1.63, filed on direct request after F11's "separate, much larger future item if ever prioritised" note was never actually turned into a tracked item. | Not started — genuinely audit-first: read MATLAB's actual `core/TRACE.m` directly (not inferred) to scope this, see companion pathways doc's `### F16` section for the full algorithm and the geometry-library roadblock found (MATLAB's `alphaShape` object's `alphaSpectrum`/`RegionThreshold`/native-3D capabilities have no ready-made Python equivalent; buildable from `alphashape`'s lower-level `alphasimplices` primitive, but as new, unverified geometry code, not a library swap). | **[Behavior-changing risk if `method` default ever flips to `"trace3"`; additive as long as it stays opt-in]** — implementing the new algorithm behind the already-existing (currently dead) `method='trace3'` option changes nothing for today's `"legacy"`-default callers. Exact MATLAB parity may not be achievable the way this repo's other ports are verified, since `alphaSpectrum`/`RegionThreshold` are undocumented MATLAB toolbox internals with no source to copy — test plan should target behavioural invariants, not a MATLAB-fixture numeric match, a materially weaker parity bar than this repo's norm, stated explicitly rather than discovered after a fixture comparison fails. 3D support is out of scope until F2 lands (no 3D `Z` exists to test against yet). |
| F17 | — (Python-only correctness gap, surfaced by F8's audit) | **Direction corrected twice — see v1.70.** Originally filed as #315 during F8 (v1.66) assuming `.contains()` (training) was the bug; v1.68 "corrected" that to the opposite claim — that MATLAB's `isinterior` is boundary-*exclusive*, so explore's `.covers()` needed to become `.contains()` — based only on confirming `isinterior` is *called* in `TRACE_legacy.m`/`TRACErescore`, not on any actual evidence of how it treats boundary points. Both directions were wrong. Independently re-verified in PR #319's review (v1.70) against MathWorks' own `polyshape.isinterior` documentation, which states boundary points count as interior — i.e. boundary-*inclusive*, matching Shapely's `.covers()`, not `.contains()`. #315's own proposed fix (explore → `.contains()`) was correctly rejected; the real bug was training's `.contains()`, fixed instead via a new `pointwise_covers()` helper (`instancespace/data/model.py`) used by `Footprint.from_polygon()`/`contra()`/`tight()`/`fit_poly()`, unifying both build and explore on boundary-inclusive `.covers()` semantics. | **Implemented in PR #319 (merged, v1.70)** — #315 closed as "proposed fix rejected, real fix landed elsewhere"; linked as a sub-issue of #260. | **[Behavior-changing]**, confirmed not just assumed — training-time footprint area/purity/contradiction resolution differs for any dataset with points exactly on a fitted polygon edge; verified via PR #319's 512-test run, not MATLAB-fixture-verified in this pass (`correct_results_simulation.csv` is a Python regression baseline, not a verified MATLAB oracle per the PR's own compatibility note). |
| F18 | — (Python-only architectural proposal, no MATLAB equivalent to port — MATLAB itself only unifies PYTHIA/TRACE this way, not PRELIM/SIFTED/PILOT) | Unify build/explore into single-body stage methods for all stages (extend the `Stage` contract so each stage owns both training and inference in one place, e.g. a `predict()` method alongside `build()`), replacing today's parallel `_explore_*` method family on `InstanceSpace` — proposed as "how someone would actually implement this from scratch," not a patch on top of the existing split. | Filed as new issue #316 ("F18 — Unify build/explore into single-body stage methods (all stages), not a parallel `_explore_*` family"), linked as a sub-issue of #260. Not implemented — filed per direct instruction ("file the one function one body for all stages as a future functionality"), explicitly scoped as a future item. See #316's body for the full benefits/costs/risks breakdown and the recommended order if ever prioritised (PYTHIA/TRACE first, since F8 already did the hard part of that split; then PRELIM; then SIFTED/PILOT last, since their `_explore_*` counterparts are already trivial passthroughs). | **[Unknown until scoped]** — a large structural rewrite touching `stages/stage.py`/`stage_runner.py`'s stage contract and every stage's public surface; blast radius and compatibility tag can't be assessed until someone actually designs the `predict()` contract in detail, which this filing deliberately doesn't do. |
...
- Parallelism is intra-stage in both (MATLAB `parfor` + shared `parpool`; Python
  ...
  `StageRunner.run_many_stages_parallel` was `NotImplementedError` and confirmed unused dead code
  (not a regression, since neither implementation runs independent stages concurrently) — removed
  entirely in a later dead-code cleanup pass rather than left as a stub.
...
- **PYTHIA (#298, 10 findings)** — verified; fixes shipped (see commits `b6508cb`, `1fe551f`, then
  issue 6 in a follow-up commit `b43b71e`). **All 10 fixed** (issues 1-9, then issue 10 last):
  issue 6 (sample weights now thread into every CV fold's fit during Sobol/Bayes candidate
  ranking, not just the final full-data fit, via a new `_cv_fit_params` helper — matches MATLAB's
  `Wtrain` threading through `evalFoldClassifier`, confirmed the ranking metric itself stays
  unweighted in both MATLAB and Python). Issue 10 (eval/skip mode) — `PythiaOptions.skip` (default
  `False`) now bypasses classifier training entirely, matching `core/PYTHIA.m`'s `opts.skip`/
  `emptyPYTHIAout`: real zscore `mu`/`sigma` still computed, every classifier-derived field
  becomes a "nothing trained" placeholder (`_ConstantClassifier(False)` per algorithm so
  `PythiaOutput.svm`'s consumers never need to special-case it, `-1` selection sentinels matching
  the existing F8 convention, a 9-column summary with no hyperparameter columns matching MATLAB's
  `buildSummary(..., [], [])`). Investigating this issue surfaced a materially different situation
  from MATLAB's own: this port's TRACE (legacy-only) clusters PYTHIA's `y_hat` predictions with
  DBSCAN to build compact footprints — `y_hat` fills the role DBSCAN's own density clustering
  plays on raw labels in true legacy TRACE — so skipping PYTHIA while `trace.use_sim=True` would
  silently degrade footprints from compact regions to fragmented ones built from raw, noisy
  `y_bin`, unlike MATLAB's trace3 (which has an independent, Yhat-free compacting mechanism and so
  degrades gracefully instead). Rather than ship that silent degradation, `InstanceSpaceOptions.
  __post_init__` now rejects the `pythia.skip=True` + `trace.use_sim=True` combination outright
  (`ValueError`, with the reasoning above in the message) — skip mode is only constructible
  alongside `trace.use_sim=False`. **#298 has no remaining open findings** and can be closed. Full
  breakdown in the GitHub comments on #298.
  - Issue 4 (`n_tuning_iter` ignored for `tuning='bayes'`; SVM's discrete Bayes search space) was
    ...
    both genuine deviations, not preserved parity. The follow-on **#304** convergence claim was
    later withdrawn as unverified: its 24/30 tolerance count used `legacy-unknown` rounded final
    metrics, not a provenance-stamped optimizer trace. R2026a PYTHIA explicitly uses
    `expected-improvement-plus` with four seed points; Python retains four seed points and plain
    EI as skopt's closest base analogue, without changing the shared 20-evaluation budget. A
    verified equal-budget, repeated-seed trace remains the prerequisite for any further optimizer
    change.
...
findings above (SIFTED issues 5/7, PILOT issues 1/3/7, and all 7 of TRACE's, none yet implemented)
remain a real backlog, tracked on their respective GitHub issues, not silently closed out by
"verification is done." CLOISTER's #299 and PYTHIA's #298 have no remaining deferred findings —
...
| F8 (explore/build code reuse, TRACE **and PYTHIA** - corrected v1.65, implemented v1.66) | **Done (v1.66)** — `test_pythia_weighted_selection_is_shared_by_build_and_explore` (`tests/test_explore_pythia.py`) breaks `PythiaStage._weighted_selection` and confirms both the training-path call (`_determine_selections`) and `_explore_pythia` reflect the broken result. TRACE's own audit found no build/explore duplication to extract (see F8's roadmap row) - no TRACE drift test needed since there's no second implementation left to diverge. |
...
| v1.59 | 2026-08-02 | Repo-wide dead-code cleanup, on request ("check whether there is dead code unused functions or settings"). Ran `vulture` (installed ad hoc for this pass, not added as a project dependency) against `instancespace/`, then individually cross-checked every flagged name against the whole repo (grep across `instancespace/`, `tests/`, `docs/`, notebooks) before touching anything - vulture's raw 60%-confidence output was mostly false positives (NamedTuple/dataclass fields consumed reflectively via the stage-runner's DAG-style argument injection, public API methods, active enum members), all correctly left alone. Removed 8 confirmed-dead items (commit `4e6bc72`): `instancespace/scripting/` (`script_fcn.py`/`script_disc.py`/`__init__.py`) entirely - every `script_fcn.py` function was a `raise NotImplementedError` stub at 0% coverage, superseded long ago by working private equivalents in `_serialisers.py` (`_draw_scatter`, `_write_array_to_csv`, etc.); `script_disc.py`'s citation printer was unreferenced anywhere, that role now served by `CITATION.cff`/`README.md`. Flagged this one to the user first via `AskUserQuestion` since F5's roadmap entry named these files by path as its future work target - user chose delete now (recommended), so F5's pathway note in `docs/python_implementation_pathways.md` now points at `_serialisers.py` instead, recording the audit finding F5 itself asked for (2D-only throughout, no dimensionality branching). Also removed: `StageRunner.run_many_stages_parallel` (`stage_runner.py`) - a `NotImplementedError` stub the roadmap's own F4 audit (Sec 6.1) already called out as "unused dead code, not a regression" but never actually deleted until now, note updated to match; `MissingOptionsError` (`data/options.py`) - custom exception never raised or caught; `InstanceSpaceOptions.to_file()`/`Metadata.to_file()` - both `NotImplementedError` stubs, never called; `PreprocessingOut` (a fully empty dataclass, `pass` body) and `PreprocessingDataChanged` (`data/model.py`) - both superseded by `stages/preprocessing.py`'s own `PreprocessingOutput` NamedTuple; `DEFAULT_PERFORMANCE_NUM_CORES` - an orphaned constant, `PerformanceOptions` has no field using it; `_write_cell_to_csv` (`_serialisers.py`) - never called, every site uses `_write_array_to_csv`; `StageProgress` dataclass + `to_dict()` (`progress_reporter.py`) - never instantiated, the real reporters build dicts directly. One item flagged, not removed: `StageStatus.PENDING` is a currently-unassigned enum member, but the class is documented as communicating status "to external systems" - removing an enum value on a guess risks breaking a consumer's contract this repo can't see, left in place. While re-verifying with the project's actual gate (`mypy --strict`, not plain `mypy` - `pyproject.toml`'s `check_mypy` task uses `--strict`), found and fixed (commit `28d7389`) a genuine gap in v1.58's own SIFTED fitness-cache addition: `instance.cost_cache[cache_key]` returned `Any` from a function declared to return `float`, missed because that commit was verified with plain `mypy`. Verified: full suite re-run after all changes, 420 passed (unchanged), coverage rose 91.33% -> 93.30% since the removed code was 0%-covered dead weight; `ruff check`/`mypy --strict`/`black --check` re-run on every touched file individually, all clean (the pre-existing 291 ruff / 9 black findings elsewhere in the repo are T9's already-tracked debt, confirmed unrelated to this change via a stash-based before/after comparison, except one genuine black-formatting fix this pass's own edit to `metadata.py` needed and got). |
| v1.60 | 2026-08-03 | Two-part audit on request: code duplication ("same purpose/structure in different files") and test-data organisation. **Code duplication:** compared `plotting.py` (Q7's interactive notebook functions) against `_serialisers.py`'s private `_draw_*` batch-PNG-export helpers - confirmed real duplicate *purpose* (scatter-by-source, scatter-by-portfolio, good/bad scatter, footprint rendering, all done twice) but not duplicate *code*, since the two serve genuinely different jobs (interactive/`Model`-driven/categorical-legend vs batch-export/raw-array-driven/MATLAB-parity-styled). Proposed consolidating only the lowest-level shared piece (`_draw_footprint`'s already-`ax`-based polygon rendering) - **declined by direct instruction, not pursued.** Also found `SiftedStage._compute_correlation`/`CloisterStage._compute_correlation` share the same algorithmic shape (nested-loop NaN-aware pairwise Pearson), independently implemented twice - initially miscast as "pairs with F3's still-open vectorization item"; **corrected by direct instruction: F3 (#263) is closed `not_planned` (won't-fix, closed 2026-07-29), not open** - this was never written into a doc, only said in chat, so no doc correction was needed, but the record here is now right: any future correlation consolidation stands on its own, not as a ride-along on already-scheduled work. Systematically checked every function name defined more than once in `instancespace/` (`report_stage_*` across the 4 `ProgressReporter` subclasses, `_run`/`_inputs`/`_outputs` across the 8 `Stage` subclasses, both `__getstate__`s, `Model.save_*` delegating into `_serialisers.py`, `InstanceSpace.plot_*` delegating into `plotting.py`) - all legitimate polymorphism or intentional thin-wrapper composition, not duplication. **Test data:** verified current `tests/test_data/`/`tests/matlab_reference/`/`examples/data/` state matches `docs/test_data_audit.md`'s existing audit exactly (Steps 1-4 done, Step 5/T10e still blocked on #278, nothing to re-derive). Hashed every file in both trees to check for duplication the original audit's file-existence-based method wouldn't catch - found `tests/test_data/trace_csvs/` is real and used but doesn't follow the `<stage>/input/,output/` naming convention every other stage uses (naming-only finding, folded into T10e's scope, no separate action), and two concrete cross-stage/cross-suite duplication patterns folded into the test-data audit doc as new **§7.1** (on request, "fold it in now"): Pattern A, pipeline-chained data - PRELIM's output is byte-identical across up to 9 private copies feeding SIFTED/PYTHIA/TRACE's own fixtures, each with a distinct confirmed reader; fix is downstream tests reading the upstream stage's own `build_data/<stage>/output/` directly, no new top-level category, already inside T10e's scope. Pattern B, genuinely shared input with no single producing stage - the same `metadata.csv` is byte-identical across `test_data/load_file/`, `test_data/preprocessing/`, and `matlab_reference/input/`, each with an independent reader; fix is a new top-level `shared_inputs/<name>/` directory (name open to revision, deliberately not "global" - collides with Python's own keyword - or "class" - already overloaded as "class label" in this codebase's ML vocabulary). Documented only, per the same #278 blocker as the rest of §7 - no fixture moved. GitHub issue #310 (T10e) updated to record the expanded scope so the migration moves fixtures once, not twice. |
| v1.61 | 2026-08-03 | Implemented #301's last remaining chunk (issues 1/3/7, `cost_weight`/`alpha` semantics) plus F2's `ntries`-parallelism sub-item (#262), per direct instruction (v1.57 had deferred both to F2; user then asked for both explicitly, confirming via `AskUserQuestion` that MATLAB's standalone PILOT stage defaults to the numerical branch with real `ntries` restarts - only SIFTED's internal GA fitness call is analytic-only/single-trial, a distinct fact from the numerical branch being "dead"). **Renamed to match MATLAB** (user's explicit choice over inventing new names): `PilotOptions.alpha` (previously overloaded to mean a precomputed solution vector) split into `precalc_alpha` (that same vector meaning, unchanged shape/behaviour) and a new `cost_weight: float = 1.0` scalar (MATLAB's `costWeight`). `analytic_solve()` now scales the performance block by `sqrt(cost_weight)` before the eigendecomposition and divides `C` by `sqrt(cost_weight)` afterward, matching MATLAB's `Xbarw`/`out.C` snippet quoted in #301's issue 3; `error_function()` now weights the performance columns' squared error by `cost_weight` (issue 7) and takes an explicit `d` (output dims, always 2 today, parameterised rather than hard-coded per the issue's own suggested fix). **Additive, not behavior-changing**: `cost_weight` defaults to 1.0, at which both changes are exact no-ops (`sqrt(1.0)=1`, weight array all-ones) - verified via `test_analytic_solve_default_cost_weight_is_unweighted`/`test_error_function_default_cost_weight_matches_unweighted` asserting bit-identical output at the default, not just "should be identical." **`ntries` parallelism:** `numerical_solve()`'s restart loop now runs on a `ProcessPoolExecutor` (not `ThreadPoolExecutor`) when `parallel_options.flag=True` - empirically benchmarked before choosing, not assumed: threads were *slower* than sequential (0.29x, GIL contention on the CPU-bound BFGS solve) while processes gave 3.92x at 4 workers on a representative PILOT problem size, contradicting the pathway doc's original suggestion to reuse Q6's `ThreadPoolExecutor`. **Nested-parallelism guard**: confirmed structurally, not just defensively, that SIFTED's GA fitness function can never trigger this pool - `SiftedStage._GA_FITNESS_PILOT_ANALYTIC = True` is a hardcoded class constant (not user-configurable), and `pilot()`'s `if options.analytic:` branch means the numerical branch (where the pool lives) is unreachable from inside SIFTED's own `pygad` worker processes regardless of `parallel_options`. Added `multiprocessing.parent_process() is not None` as a cheap circuit-breaker in the pool-opening code anyway, as insurance against that hardcoded constant ever being changed without the consequence being noticed, not because the bug is currently reachable. Each restart submitted as an independent, picklable unit of work (new `PilotStage._solve_one_trial`); verified order-independence directly (`test_numerical_solve_parallel_matches_sequential` asserts bit-identical `idx`/`alpha`/`eoptim`/`perf` whether `parallel_options.flag` is `True` or `False`) and the nested-guard itself (`test_numerical_solve_skips_pool_when_already_in_worker_process`, mocking `multiprocessing.parent_process` non-`None` and asserting `ProcessPoolExecutor` is never constructed). `PilotInput` gained a `parallel_options: ParallelOptions` field (mirroring `TraceInputs`/`PythiaInput`'s existing pattern, wired automatically via `_InstanceSpaceInputs`'s shared field); `pilot()`/`numerical_solve()`'s own `parallel_options` parameter defaults to `None` (sequential) so all ~20 existing direct test call sites needed no changes. New `PilotOptions.default(cost_weight=..., precalc_alpha=...)` factory params; `costweight` added to the JSON options loader's field-name mapping (`instancespace/data/options.py`), matching MATLAB's `costWeight` naming; `_check_positive("pilot.costWeight", ...)` added to `InstanceSpaceOptions.validate()`. **Verification note:** hit a real false alarm mid-session worth recording - combining `test_build_pilot.py` with `test_build_pilot_pythia.py` in one pytest invocation appeared to hang (24+ min, CPU-pegged); root-caused by stashing all changes and reproducing the identical timeout on unmodified baseline code, proving it's pre-existing slowness in `test_build_pilot_pythia.py`'s four `tuning='bayes'` tests (`n_cores=8` joblib/skopt search, independent of this session's changes), not a deadlock this session's `ProcessPoolExecutor` usage introduced. Full suite (423 passed, 4 deselected = those pre-existing slow tests, 431s) confirmed clean separately. GitHub: posted the fixed/breakdown comment on #301 and closed it (all 7 findings now resolved across this and the earlier v1.58 pass); posted a progress comment on #262 (F2 parent) noting the `ntries`-parallelism sub-item is done while the 3D/viewpoint/PLS work `#262`'s own body describes remains unstarted. |
| v1.62 | 2026-08-03 | Continued #300/#298 per direct instruction ("continue with 300 and 298"). **Bookkeeping correction first**: #300's second comment (v1.58's own pass) wrongly said "Untouched, remain open: issues 1, 3, 8, 9" - re-verified directly against the current code (`select_features_by_performance`'s sorted-threshold logic, `_find_best_combination`'s `ga_instance.selfx = x_aux` wiring, `evaluate_cluster`'s `min_clusters = 3` + corrected logging index, the density-filter branch's `[:, selvars]` slicing) and confirmed all four were already fixed in commit `a3e2859` (v1.58's *first* comment already said so correctly) - the "remain open" line was simply wrong, posted as a correction comment on #300 rather than left standing. **#298: nothing to do** - its only remaining item (issue 10, eval/skip mode) is architecture-blocked on F8/F9, not a scope choice, confirmed via direct question to the user rather than assumed. **#300 issues 5 and 7, both approved by direct instruction (recommended options)**: Issue 5 - `SiftedStage.cost_fcn`'s k-NN fitness now scores `scoring="accuracy"` and computes `loss = 1.0 - scores.mean()` instead of `neg_mean_squared_error` on the binary good/bad labels (a regression metric applied to a classification target), matching MATLAB's `fitcknn`/`kfoldLoss` classification loss - same aggregation structure preserved (`y = max(y, loss)` across algorithms), only the per-algorithm metric changed. Issue 7 - `evaluate_cluster` and `select_features_by_clustering` both now cluster z-scored (population-normalised) feature vectors via a new `SiftedStage._standardize_for_correlation_distance()` helper before calling `KMeans`, rather than raw vectors under Euclidean distance - derived and verified the underlying identity directly (`||u-v||^2 = 2n*(1-corr(u,v))` for two population-z-scored length-`n` vectors, a positive monotonic transform of Pearson correlation distance), so ordinary Euclidean k-means on the standardized vectors reproduces a correlation-distance k-means's nearest-centroid assignment - chosen over hand-rolling a Lloyd's-algorithm variant to avoid re-deriving sklearn's own well-tested convergence/empty-cluster handling. `silhouette_score`'s existing `metric="correlation"` call (already correct) is unchanged - only the clustering *input* changed, closing the inconsistency between the two. **[Behavior-changing]**, per both issues' own audit findings: SIFTED's GA search direction (issue 5) and feature clustering (issue 7) can now select different final feature sets than before. Verified: full `test_build_sifted.py` (13 passed, 4 new regression tests: `_standardize_for_correlation_distance`'s zero-mean/unit-variance and constant-feature handling, `select_features_by_clustering`'s actual input to `KMeans.fit_predict` confirmed standardized via a monkeypatch capture, `cost_fcn`'s classification-loss formula confirmed via mocked `cross_val_score`/`PilotStage.pilot`), full project suite (423 passed, 4 pre-existing-slow deselected, 445s, unchanged from the previous run's count). **New finding, filed not fixed** (#312): while implementing issue 5, noticed `pygad.GA` always *maximizes* `fitness_func`'s return value (confirmed directly: `best_solution()` picks `numpy.max(pop_fitness)`), but MATLAB's `ga()` always *minimizes* - and `cost_fcn` returns a positive "worse is bigger" loss with no final sign flip, which `SiftedOptions.stop_criteria`'s supported `"reach_0"` value only makes sense for if fitness were negative-and-approaching-zero. Circumstantial evidence of a real sign-convention bug (a classic direct-port-without-inverting-the-sign mistake), but not confirmed or fixed here - a bigger, independent-verification-worthy change than either approved issue, and not something to fold in silently. GitHub comments posted on #300 (correction + issues 5/7 fixed) and #298 (issue 10 status confirmed, no action); #312 filed for the sign-convention finding. |
| v1.63 | 2026-08-03 | Resolved #312 (the sign-convention finding from v1.62), per direct user insight ("if we are using accuracy, and maximising, isn't it easier to maximise the minimum accuracy instead of doing these sign flips?") rather than the more involved "compare GA trajectories with/without a sign flip" verification #312 originally proposed. `cost_fcn` now tracks `y = min(y, scores.mean())` (the worst/lowest per-algorithm accuracy, starting `y = np.inf`) and returns it directly - no `loss = 1 - accuracy` concept, no sign flip. Mathematically equivalent to the "correctly sign-flipped" version of v1.62's own formula: for `-max_i(loss_i) = -max_i(1-acc_i) = min_i(acc_i) - 1`, maximizing `min_i(acc_i)` and maximizing `-max_i(loss_i)` (MATLAB's actual `minimize(max_i(loss_i))` objective, restated for pygad's maximize-only convention) share the same optimum, differing only by an additive constant - so this formulation is provably correct by construction rather than needing an empirical sign check. Updated `test_cost_fcn_uses_classification_accuracy_loss` to use two algorithms with different accuracy (0.9, 0.7) so the assertion (`fitness == 0.7`, the minimum) actually distinguishes min from max, rather than the single-algorithm version that couldn't. Verified: `test_build_sifted.py` (13 passed, including the MATLAB-comparison `test_run`, which still passes with the corrected search direction), full project suite (427 passed - up from 423 since this run includes v1.62's own new tests for the first time - 4 pre-existing-slow deselected, 436s). #312 can be closed - the sign-convention concern is resolved, not merely worked around. |
| v1.64 | 2026-08-03 | Filed **F16** (#313, sub-issue of Phase F's #260) on direct request, after the user asked "did we miss TRACE3 or is it in one of these tasks?" during a priority review - answer: neither, it was explicitly scoped out of F11 ("porting TRACE3 itself would be a separate, much larger future item if ever prioritised") but that future item was never actually turned into a tracked issue until now. Before writing the scope, added and read the actual MATLAB source (`andremun/InstanceSpace`'s `core/TRACE.m`, already cloned at `/workspace/instancespace` this session) rather than working from the roadmap's existing prose summary alone - confirmed TRACE3's exact algorithm (`TRACEbuild3`'s alpha-shape-build-then-iteratively-tighten-via-alphaSpectrum loop, `TRACEmetrics3`, `TRACErescore`, the `TRACE.m` dispatcher), and found the real roadblock this item hinges on: MATLAB's `alphaShape` object's `alphaSpectrum`/`RegionThreshold`/native-3D capabilities have no ready-made Python equivalent - `alphashape` 1.3.1 (already a dependency, used today only for legacy's single fixed-alpha call) exposes low-level primitives (`alphasimplices`, `circumradius`, `scipy.spatial.Delaunay`-based, dimension-agnostic) that could build an equivalent, but as new, unverified geometry code, not a library swap. Also confirmed via direct code check (not assumed): Python's `TraceInputs` already has `y_hat` wired through (not a blocker), and found two smaller pre-existing gaps while checking - `TraceOptions` has no `min_instances`/`min_area_frac` fields (F11 deliberately left them out), and `TraceOptions.purity` (Python's name for MATLAB's `PI`) defaults to `0.55` against MATLAB's actual `ISAdefaults.m` default of `0.6`. Explicitly flagged that exact MATLAB parity may not be achievable here the way this repo's other ports are verified, since `alphaSpectrum`/`RegionThreshold` are undocumented MATLAB toolbox internals with no source to copy - test plan should target behavioural invariants, not a fixture-numeric-match, stated up front rather than discovered after a comparison test predictably fails. Added `### F16` to `docs/python_implementation_pathways.md`, a table row + "Recommended order" row 11 here (soft preference: after F8, since `TRACErescore` is F8's own explore/build-unification problem; 3D phase hard-blocked on F2). No code changed. |
| v1.65 | 2026-08-03 | Two-part request: implement PLS for PILOT (2D only, but written so 3D needs no rework), then audit F8/F9 for complete scope "including PYTHIA, not just TRACE." **PLS (`PilotOptions.method`, F2/#262):** added `method: str = "standard"` (matching MATLAB's `opts.method` naming exactly, core/PILOT.m, read directly - `andremun/InstanceSpace` was already cloned this session at `/workspace/instancespace`), checked before `analytic` in `pilot()`'s dispatch (PLS ignores `analytic` entirely, matching MATLAB). New `PilotStage.pls_solve()` uses `sklearn.cross_decomposition.PLSRegression(n_components=dims, scale=False)` - `scale=False` because MATLAB's `plsregress` only mean-centres, never scales to unit variance, unlike sklearn's own `scale=True` default. Written to take `dims` as a parameter (verified working unmodified at `dims=3` with a direct test) so a future public `dims` option needs no change here. **Caught and fixed a real correctness bug before it shipped**, not just implemented the obvious mapping: MATLAB's `out.A = stats.W'` is documented as "used by exploreIS to reproject new instances via `Z=X*A'`" - that identity holds for MATLAB's `plsregress` (SIMPLS algorithm, deflates cross-covariance, not X) but does **not** hold for sklearn's `PLSRegression` (NIPALS-based, deflates X across components) - verified empirically (`x_weights_` gives ~0.16 max reprojection error at 3 components; `x_rotations_`, which sklearn exposes specifically for this, gives ~1e-16). Used `x_rotations_` for `out_a`, not `x_weights_`, to make the property MATLAB's own comment describes actually hold for this port. Flagged that exact bit-for-bit MATLAB parity isn't expected regardless (SIMPLS vs. NIPALS are related but different algorithms), matching the same "verified-equivalent, not identical" bar F16 already established for TRACE3. Also added `_check_member("pilot.method", ...)` validation and a `sklearn.cross_decomposition` mypy override entry. **[Additive]** - `method` defaults to `"standard"`, identical to today's behaviour; all 20 pre-existing PILOT tests pass unchanged. 6 new tests (shapes, the reprojection-property regression test, dims=3 genericity, end-to-end dispatch, options default). **F8/F9 scope audit, corrected via direct MATLAB source reading (`core/PYTHIA.m`, `InstanceSpace.m`'s `evaluateTestSet`), not assumed:** F8's existing text ("S1 resolved the PYTHIA half... nothing left to reconcile") was wrong - `InstanceSpace._explore_pythia` still independently reimplements `PythiaStage._determine_selections`'s precision-weighted selection formula and `_compute_znorm`'s z-scoring, both real, unaddressed duplication S1 never touched (it only fixed how the classifier itself is called). F9's existing text was incomplete, not wrong - it covered PRELIM's ground-truth labels but not MATLAB's `PYTHIAevalMode`, which computes real `accuracy`/`precision`/`recall` against ground truth using the already-trained classifiers and reuses the same `computeSelection` subfunction training uses - "full evaluation scope" needs this, confirmed directly from MATLAB source, not inferred. F8's PYTHIA-side extraction and F9's metric computation are two views of the same code, sharpening F8-before-F9 into a closer-to-real dependency for that half specifically. Updated both `### F8`/`### F9` pathway sections, both roadmap table rows, and the Phase T checkpoint table's F8 row (previously claimed the PYTHIA half was "resolved by S1 instead" - also corrected). No code changed for F8/F9 themselves - audit and scope correction only, per this repo's audit-first convention for F-items. GitHub comments posted on #268 and #269 with the corrected scope. Full suite re-verified after the PLS change: 433 passed (up from 427), 4 pre-existing-slow deselected, 432s, no regressions. |
| v1.66 | 2026-08-03 | Implemented F8 (#268) and F9 (#269), following v1.65's corrected scope, then ran a post-implementation MATLAB-parity audit per direct instruction. **F8:** `PythiaStage._determine_selections`'s precision-weighted formula split into a new shared `_weighted_selection(nalgos, precision, y_hat)` static method, called by both `_determine_selections` (training) and a rewritten `_explore_pythia` (explore) - `_explore_pythia` also switched to `PythiaStage._compute_znorm()` for its mu/sigma instead of an inline copy of the same formula. TRACE audited directly against this same "shared function" goal and found **no live duplication to extract** - `_explore_trace()` already correctly reuses the trained `Footprint.polygon` rather than re-deriving footprint math (see corrected pathway doc for the full reasoning); the audit surfaced a genuine, separate, previously-undocumented inconsistency instead (training's `Footprint.from_polygon`/`contra()` use boundary-exclusive `.contains()`, explore's `_explore_trace` uses boundary-inclusive `.covers()` matching MATLAB's `inpolygon`) - filed as **#315**, not fixed, since changing it alters existing training-time `density`/`purity` output. New drift-detection test (`test_pythia_weighted_selection_is_shared_by_build_and_explore`) breaks the shared function and confirms both call sites reflect it. The previously-flagged-as-risky `nalgos == 1` explore-path gap is now tested (`test_pythia_selection0_nalgos_equals_one`) and confirmed to reproduce a **separate, pre-existing bug** (selection index `1`, not `0`, for a single-algorithm portfolio - present in the original `_determine_selections` before F8, not introduced by the extraction) - filed as **#314**, not fixed here, since it's its own `[Behavior-changing]` fix needing independent verification. **F9:** `compute_binary_performance(y_raw, perf_opts, general_opts, log_prefix)` extracted from `PrelimStage._prelim()` into `prelim.py` (`_select_best_algorithms` removed as a separate method, its tie-breaking folded in); `PrelimStage._prelim()` and a new `InstanceSpace._explore_evaluate()` both call it, the latter with `log_prefix="EXPLORE"` (matching MATLAB's own "[EXPLORE] Calculating the binary measure of performance" line for free). New `InstanceSpace._build_test_algo_matrix()` reindexes a test set's `algo_*` columns to the trained algorithm order case-insensitively (`strcmpi`-style); an algorithm in training but absent from the test set becomes an all-NaN column (excluded from ever being "best" via `compute_binary_performance`'s existing NaN handling) and reports `NaN` accuracy/precision/recall/confusion-matrix rather than one computed against a fabricated label; a "new" test-only algorithm is dropped (deferred, as decided in v1.65). `_explore_evaluate()` computes real per-algorithm accuracy/precision/recall/confusion-matrix (`sklearn.metrics`, matching MATLAB's `tp/(tp+fp)`/`tp/(tp+fn)`/`(tp+tn)/ninst`) from PYTHIA's already-computed `y_hat` against this ground truth - the corrected "real metrics, not just labels" scope from v1.65. New `ExploreStage.EVALUATION`, yielded by `explore_stage_iter()` only when `len(test_metadata.algorithm_names) > 0`; `ExploreResult` gained 8 new `\| None` fields (`y_actual`/`y_best_actual`/`p_actual`/`beta_actual`/`accuracy_actual`/`precision_actual`/`recall_actual`/`cvcmat_actual`), all `None` in the feature-only case (verified by extending the existing no-ground-truth orchestration test rather than assuming it). 9 new tests: `tests/test_explore_evaluate.py` (6, numerics - reindexing, missing-algorithm NaN handling, new-algorithm dropping, metrics cross-checked against an independently-computed `sklearn.metrics.confusion_matrix`), `tests/test_explore_pythia.py` (2, the F8 drift test and `nalgos==1` case), `tests/test_explore_stage_iter.py` (1, `EVALUATION` orchestration). Full suite: **442 passed (up from 433, exactly the 9 new tests), 4 pre-existing-slow deselected, 432s, no regressions.** **Post-implementation MATLAB-parity audit** (per direct instruction, excluding strict 3D expansion and TRACE-correctness gaps per the user's own framing - see the session's final report for the full breakdown): F8/F9 close the last non-3D, non-TRACE-audit functional gaps this pass could find; F16 (TRACE3 port, #313) is a genuinely separate, still-unimplemented item - not covered by "the TRACE audit is being handled separately," which refers to #302's correctness-finding backlog, not to porting a whole new algorithm variant - flagged explicitly rather than silently folded in. GitHub: #314 and #315 filed (new findings); #268/#269 to be closed with a summary comment. |
| v1.67 | 2026-08-03 | Two-part direct request following the build/explore architecture comparison: simplify PRELIM's own build/explore duplication now, and implement F9's previously-deferred "new algorithm absent from training" edge case, at full MATLAB parity (user's explicit choice over an evaluation-metrics-only scope, after being shown MATLAB's `PYTHIAevalMode`/`TRACE`'s eval branch widen *every* output - `Yhat`/`Pr0hat`/`good`/`best`, not just `precision`/`recall`/`accuracy` - to include a placeholder column per new algorithm). **PRELIM simplification:** new shared `apply_bound_clip()`/`apply_boxcox_zscore()` pure functions in `prelim.py`, verified bit-for-bit equivalent to the code they replace (`np.clip` vs. the old mask-multiply arithmetic, including NaN handling; `stats.zscore` vs. manual `(x-mu)/sigma` at already-fit mu/sigma) before swapping - both `PrelimStage._bound()`/`_normalise()` (training) and a rewritten `InstanceSpace._explore_prelim()` (explore) now call them, instead of `_explore_prelim` hand-duplicating the same arithmetic in a separate copy. **Two real bugs fixed as an unavoidable consequence of writing the shared function correctly, not silently bundled in:** (1) `_normalise()` computed `min_x` via plain `np.min`, which - confirmed empirically - propagates NaN across an *entire* feature column from a single missing value (`x - min_x` where `min_x=NaN` poisons every row, not just the already-missing ones), unlike MATLAB's `min(X,[],1,'omitnan')`; fixed to `np.nanmin`, matching MATLAB and the unconditional `nanmin` already used elsewhere in `_prelim()` for the same statistic. (2) `_explore_prelim()` ignored `BoundOptions.flag`/`NormOptions.flag` entirely, always clipping and normalising regardless of what the trained model actually used - wrong outright when either was `False`, and unsafe when `norm=False` specifically (`lambda_x`/`mu_x`/`sigma_x` are unfit all-zero arrays in that case, so the old code's unconditional Box-Cox-at-`lambda=0` then divide-by-`sigma_x=0` would have produced `inf`/`nan` for every test instance); fixed by reading `self._options.bound.flag`/`.norm.flag` (the same `InstanceSpaceOptions` used to train the model) and skipping each step when its flag is off. Both fixes verified via new regression tests (`test_prelim_skips_clipping_when_bound_flag_is_false`, `test_prelim_skips_normalisation_when_norm_flag_is_false`); all pre-existing `test_bound`/`test_normalise`/`test_prelim_matches_matlab` tests pass unchanged (the shared-function swap itself is output-preserving by construction, confirmed by direct numeric check before the swap, not just by the tests passing). **F9 new-algorithm support, full MATLAB parity:** new `InstanceSpace._find_new_algorithms()` (case-insensitive, first-encounter order, mirrors MATLAB's `strcmpi` reconciliation) and `_build_test_algo_matrix()` extended to append real performance columns for test-set-only algorithms (previously dropped entirely) rather than just NaN-padding trained-but-absent ones. `_explore_pythia()`/`_explore_trace()` gained an `n_new_algos` parameter that widens `y_hat`/`pr0_hat` (`False`/`0.0` - no trained classifier) and `in_good`/`in_best` (`False` - no trained footprint), matching MATLAB's `PYTHIAevalMode`/`TRACEthrow3` padding exactly; new algorithms get zero precision before the weighted-selection step, so `selection0` can never point at one (matching MATLAB's `selPrecision` zero-padding). `explore_stage_iter()` restructured to reconcile new algorithms *before* running PYTHIA/TRACE (matching MATLAB's own ordering in `evaluateTestSet`), not after. `_explore_evaluate()` widened to report `NaN` accuracy/precision/recall/confusion-matrix for new algorithms (no trained classifier to score, matching MATLAB's "no CV model" convention) while still letting them participate as full candidates in `y_best_actual`/`p_actual`/`beta_actual` (a new algorithm's real performance data can make it "the best" for an instance, confirmed by a dedicated test). New `ExploreResult.algo_labels: list[str] \| None` field (trained order + new order, in that sequence) - `None` in the feature-only case, since without it the widened columns would be uninterpretable. **Confirmed via direct MATLAB source reading, not assumed:** `evaluateTestSet`'s own Box-Cox/z-scored `Y` (including its `autoNormalize` call for new algorithms) is computed but never read again - `PYTHIA`/`TRACE` are called with `Yraw`/`Ybin`/`Ybest` instead - so no Python equivalent of `autoNormalize` was needed; only the discrete ground-truth fields matter downstream, already covered by the widened `compute_binary_performance` call. 13 new tests across `tests/test_explore_evaluate.py` (7: case-insensitive reindexing, excluding vs. appending a new algorithm, `_find_new_algorithms` dedup, metrics for a new algorithm with `p_actual` correctly picking it as best), `tests/test_explore_pythia.py` (1, widening), `tests/test_explore_trace.py` (1, widening), `tests/test_explore_prelim.py` (2, bound/norm flag regressions), plus the PRELIM shared-function equivalence checks run directly (not committed as tests, verified interactively before the swap). |
| v1.68 | 2026-08-03 | Two filing requests, per direct instruction ("file the TRACE contains/covers fix as its own scoped task. also file the one function one body for all stages as a future functionality"), no code changed. **F17** (TRACE `.covers()`/`.contains()` fix, #315): before filing, re-read MATLAB's `TRACE_legacy.m` and `TRACErescore` directly rather than trust F8's (v1.66) own recorded direction — found that recorded direction was backwards. Both MATLAB functions use `isinterior` (boundary-exclusive, matching Shapely's `.contains()`), never `inpolygon` (boundary-inclusive) — so the actual fix is `InstanceSpace._explore_trace()`'s `.covers()` → `.contains()`, not `Footprint.from_polygon`/`contra()` as originally recorded. Posted a corrected-finding comment on #315 with the MATLAB evidence and a concrete 4-step implementation plan, retitled the issue to name the correct fix ("F17 — Fix `_explore_trace()`'s `.covers()` to `.contains()`, matching MATLAB's `isinterior`/`TRACErescore`"), and linked it as a sub-issue of Phase F's #260. Added a table row here. Not implemented — filed as scope only, flagged `[Behavior-changing]` since it changes which boundary-exact instances count as "in good"/"in best" at explore time. **F18** (new architectural proposal, #316): filed "unify build/explore into single-body stage methods for all stages" — extending the `Stage` contract so each stage owns both training and inference in one place (e.g. `predict()` alongside `build()`), replacing today's parallel `_explore_*` method family on `InstanceSpace` entirely, framed explicitly as "how someone would actually implement this from scratch" rather than a patch on the existing split. Noted MATLAB's own precedent is partial, not universal — `PYTHIA()`/`TRACE()` already dispatch on a single function for both modes, but `PRELIM`/`SIFTED`/`PILOT` don't, so this proposal goes further than porting MATLAB as-is. Body includes benefits, costs/risks (blast radius on `stages/stage.py`/`stage_runner.py`, PRELIM's need for read-access to already-fit parameters as `predict()` input, SIFTED/PILOT's near-triviality since their `_explore_*` counterparts are already thin passthroughs, multi-session migration effort) and a recommended order if ever prioritised (PYTHIA/TRACE first — F8 already did the hard part of that split — then PRELIM, then SIFTED/PILOT last). Linked as a sub-issue of #260. Added a table row here, tagged `[Unknown until scoped]` since the `predict()` contract itself isn't designed yet. Both issues are explicitly future/unimplemented, matching the user's own framing ("file... as its own scoped task" / "file... as a future functionality"). |
| v1.69 | 2026-08-11 | Resolved #298's last remaining item (issue 10, eval/skip mode), per direct instruction ("revise 298. if feasible, let's complete it now so it can close"). Investigation surfaced a design question worth pausing on before implementing (the user's own framing: "investigate," not "implement" - see the session transcript for the full back-and-forth): MATLAB's `opts.skip` exists as a training-cost shortcut (skip PYTHIA's classifier training when only TRACE's ground-truth footprints are wanted), and MATLAB's TRACE degrades gracefully without `Yhat` (trace3 falls back to `Zu={yi=1}` with a warning; true legacy TRACE never reads `Yhat` at all). This port's TRACE (legacy-only, per F11) is architecturally different in a way that matters here: it clusters PYTHIA's `y_hat` predictions with DBSCAN to build footprints, and `y_hat` is what makes that clustering produce *compact* regions rather than fragmenting on raw per-instance label noise - a role MATLAB's own newer trace3 fills a different way (alpha-shape tightening), and true legacy TRACE never needed to solve at all. Confirmed directly against `TraceStage.build()` (DBSCAN clusters exactly the point set it's handed, nothing more) rather than assumed. **Implemented**, matching MATLAB's `core/PYTHIA.m` `opts.skip`/`emptyPYTHIAout`/`buildSummary(...,[],[])`: new `PythiaOptions.skip` (default `False`, `DEFAULT_PYTHIA_SKIP`) short-circuits `PythiaStage.pythia()` right after resolving/validating `opts.classifier` and computing real zscore `mu`/`sigma` (both needed even in skip mode, matching MATLAB's own ordering) - `pythia()` itself split into a thin dispatcher plus a new `_train()` static method carrying the previously-inline training body, so the added `if opts.skip` branch didn't push `pythia()` over the project's branch-count lint gate. New `PythiaStage._empty_output()` builds the "nothing trained" `PythiaOutput`: `svm` holds one never-fitted `_ConstantClassifier(False)` per algorithm (reusing the existing degenerate-label sentinel rather than inventing a new one, so every consumer of `PythiaOutput.svm` still gets something `.predict()`-shaped), `y_sub`/`y_hat` all-`False`, `precision`/`accuracy`/`recall`/hyperparameters all-NaN, `selection0`/`selection1` both `-1` (Python's established F8 "no selection" sentinel, translating MATLAB's `zeros(ninst,1)` into a convention that doesn't collide with "algorithm 0 selected"). `_generate_summary`'s `param1_label` parameter widened to `str | None` (previously required) so skip mode's summary can drop both hyperparameter columns entirely, matching MATLAB's `buildSummary(..., [], [])` 9-column output exactly (verified via a dedicated test asserting the summary's column list). Rather than let the compacting-degradation risk ship silently, `InstanceSpaceOptions.__post_init__` (F13) now rejects `pythia.skip=True` combined with `trace.use_sim=True` outright (`ValueError`, message states the DBSCAN/`y_hat` reasoning above) - skip mode is only constructible alongside `trace.use_sim=False`, an explicit acknowledgment that Python's skip-mode footprints are expected to be rougher than MATLAB's own graceful-degradation case, not a silent quality regression. New tests: `test_skip_mode_bypasses_training_and_returns_empty_output` (`test_build_pythia.py`), `test_pythia_skip_with_trace_use_sim_raises`/`test_pythia_skip_with_trace_use_sim_false_constructs` plus a `pythia.skip` logical-field entry in the existing parametrized invalid-field table (`test_options_validation.py`). **[Additive]** - `skip` defaults to `False`, at which every existing caller's PYTHIA output is byte-for-byte unchanged (the new code path is only reachable by explicitly setting the new option, and the F13 guard only fires for that same non-default combination). Verified: full project suite (`poetry run poe test`) after `ruff`/`mypy --strict`/`black` all clean on every touched file. Separately, while re-reading `PythiaStage._generate_summary`'s call site to build skip mode's own call correctly, noticed the real call site passes `accuracy_record, precision_record, recall_record` positionally into parameters *named* `precision, accuracy, recall` (swapped) - the resulting `CV_model_accuracy`/`CV_model_precision` DataFrame columns may hold each other's values in the exported summary/CSV. Not caught by the existing `test_bayes_opt_gaussian`/`test_bayes_opt_poly` tests, which check `PythiaOutput.accuracy`/`.precision` directly (populated correctly during training, upstream of the swap) rather than the summary DataFrame the bug actually lands in. Flagged, not fixed here (out of scope for #298 issue 10, and CLAUDE.md's audit-before-fix discipline applies) - filed as its own issue. |
| v1.70 | 2026-08-17 | Completed the open-issue correctness pass on `codex/open-issue-big-rocks`: fixed #302, #314, and #317; rejected #315's boundary-exclusive premise using MATLAB source and reference fixtures; repaired audited PRELIM, SIFTED, TRACE indexing, serialization, parallel, and staged-model lifecycle contracts. Added focused regressions and recorded all results in `implemented_fixes.md` and `pending_issue_backlog.md`. Behavioral result: 512 tests passed when process-pool permissions were available. |
| v1.71 | 2026-08-17 | User asked for a review of PR #319 (`codex/open-issue-big-rocks`), branch/PR review requested directly. First-pass review flagged the training-side `.contains()`→`.covers()` switch (`Footprint.from_polygon`/`contra()`/`tight()`/`fit_poly()`) as reversing F17/#315's already-recorded decision, on the strength of that decision having been "confirmed by direct source read." User pushed back, citing the PR's own documentation. Re-verified independently rather than trust either side: fetched MathWorks' actual `polyshape.isinterior` documentation (blocked via direct `WebFetch`, retrieved via `WebSearch` instead), which states plainly that boundary points count as interior - i.e. `isinterior` is boundary-**inclusive**, matching Shapely's `.covers()`. This means the *original* F17 finding (v1.68) was wrong, not the PR: reading `TRACE_legacy.m`/`TRACErescore` only confirms `isinterior` is *called*, not how it treats boundary points, and that gap between "read the source" and "verified the semantics" is what let the wrong direction stand through one prior correction pass already. Retracted the review finding, confirmed the PR's `pointwise_covers()` fix (proper per-point iteration, not the old `contains(MultiPoint)` scalar-collapse bug) is correct, and flagged one real but non-blocking issue: `docs/remediation_plan.md`'s #315 row tags "No code behavior change" - on reflection this is defensible as written (it describes rejecting #315's *own* proposed fix, while the actual geometry change is already correctly tagged `[Behavior-changing]` under #302's row in the same table), so left as-is rather than "fixed" on a low-confidence read. User approved merging; merged PR #319 (commit `324830c`) into `v0.9.0/development-branch-QSF`. Found none of #302/#314/#315/#317 auto-closed despite the PR body's `Fixes #NNN` keywords - GitHub only auto-closes on merge into a repo's *default* branch, not an arbitrary base branch - so closed all four manually with comments recording the actual resolution (#315's own proposed fix correctly rejected; the real bug fixed elsewhere). Corrected the stale claim this session's own earlier work had propagated: `CLAUDE.md`'s F17 paragraph and this document's F17 table row (§6, F-phase table) both said `isinterior` was boundary-exclusive - both rewritten to record the corrected, documentation-backed conclusion, with an explicit note for future sessions not to re-trust a "confirmed by source read" claim about a MATLAB builtin's own semantics without an actual citation or empirical check. Lesson for future sessions, stated directly: a previously-recorded "verified" conclusion is not itself evidence - when a PR or user disputes it, re-derive the answer from a primary source before deferring to either side. |
| v1.72 | 2026-08-20 | Corrected #304 after an R2026a source and fixture-provenance audit. The earlier 24/30 convergence claim used `legacy-unknown` rounded final metrics, not an optimizer trace, and MATLAB explicitly selects expected-improvement-plus rather than the previously recorded per-second variant. Kept the shared 20-evaluation default and Python's four-seed/plain-EI closest analogue. **[Behavior-changing]** PYTHIA now mirrors MATLAB's one-based `seed + i` RNG boundary per algorithm and returns its actual per-algorithm CV splitters (`None` for degenerate or skipped algorithms) instead of one representative splitter. A verified equal-budget, repeated-seed optimizer trace remains pending before any further default or acquisition change. |
| v1.73 | 2026-08-21 | **[Behavior-changing]** PILOT's default restart count changed from the former Python value of 5 to MATLAB R2026a's `opts.pilot.ntries = 10`. The extra starts can select a different numerical projection and double the default restart budget; callers needing the earlier Python behavior can explicitly set `n_tries=5`. Verification pins both the ten-restart default and the explicit five-restart override, checks the value against `utils/ISAdefaults.m`, replays the verified R2026a PILOT fixtures, and runs the complete release validation suite. Also removed the unused `_validate_explore_trace_dimensions()` compatibility hook: the fitted `TraceStage.predict()` remains the single owner of dimension validation at the lazy TRACE boundary, after PYTHIA is yielded, so this cleanup is **[Additive]** and does not change outputs or stage order. |
| v1.74 | 2026-09-25 | **[Additive]** Resolved #325 and #326 (docstring format and API-docs tooling). #326: commit `0a7999b` had already changed the `format_docstrings` path from `matilda/` to `instancespace/`. But `docconvert` reads only reST and epytext input. A test run made no change to the hybrid docstrings, with the old path or the new path. This change removes the task and the `docconvert` dev dependency. #325: this change converts the hybrid Google/numpy sections to numpy format in all affected modules. `Args` becomes `Parameters`. `name (type): text` becomes `name : type` with an indented description. `Type: text` becomes `Type` with an indented description. Headers with a trailing colon, such as `Returns:`, and the misspelled `Return` are also fixed. After the conversion, ruff D417 and D414 found parameters without descriptions and empty sections. This change adds the descriptions and removes the empty sections. It also removes the `_description_` and `TODO THIS` placeholders. The stage summary in the package docstring now agrees with Smith-Miles & Muñoz (2023). All new and changed docstring text follows the `ste-writing` skill (STE-flavored mode). The new `poe docs` task (`pdoc instancespace -d numpy -o site`) runs in `poe check`, `poe test`, `validation-tests.yml`, and `docs-pages.yml`. `docs-pages.yml` changes from `-d google` back to `-d numpy`. pdoc exits with a non-zero code if it cannot parse a docstring. Thus, these builds stop the same drift in the future. Verification: `pdoc -d numpy` builds all modules. ruff, black, and mypy --strict pass. pytest: 1045 passed, 1 failed. The failed test is `test_pythia_matches_legacy_matlab_snapshot`: one `pr0_hat` element in 2350 differs by 1.02e-13, and the tolerance is `atol=1e-13`. The same test also fails on the unchanged base commit, so this change did not cause the failure. This change has no effect on runtime code. |
| v1.75 | 2026-09-26 | **[Additive]** Gave the hosted docs a curated landing page, Getting Started guide, and Options Reference, at the user's direct request to make them read less like a raw pdoc dump and closer to `andremun/InstanceSpace`'s newly rebuilt (2026-09-25, its #53) hand-authored reference site. Full parity was scoped down, on the user's explicit choice among three options, to a "curated shell + pdoc underneath": three hand-written pages (`docs_site/pages/*.html`, styled by `docs_site/assets/style.css`) plus pdoc's existing generated API reference, not a full port of the MATLAB repo's 29-page, hand-written-per-function site and its 664-line generator. `poe docs` now builds pdoc's output into `site/api/` (previously `site/`) and copies the curated pages into `site/`. A small pdoc template override (`docs_site/pdoc-template/module.html.jinja2`) adds a "Docs Home" link from every generated API page back to the new landing page; both halves share one accent color (pdoc's own default link blue) so they read as one site without a full pdoc reskin. The Options Reference's content is adapted from README's existing "## Options" section (kept in the README too - normal doc/README duplication, not the kind of redundant-code-field smell this repo's own review checklist warns about) and written in STE, per this repo's prose convention. README and CONTRIBUTING now point to and explain the new pages. Verification: `poe check` (black, ruff, mypy --strict, docs) passes; all internal links between the three pages, `site/assets/style.css`, and `site/api/instancespace.html` resolve; a Playwright screenshot of each of the three pages and one generated API page confirms visual rendering and the "Docs Home" link's placement. No runtime code touched. |
| v1.76 | 2026-09-26 | **[Behavior-changing]** Fixed #345: `InstanceSpace.explore()`'s PYTHIA evaluation no longer fabricates `accuracy`/`precision`/`recall`/confusion-matrix scores for a trained algorithm the test set has no data for. That behavior was a deliberate bit-for-bit mirror of a real MATLAB defect (`andremun/InstanceSpace#58`), recorded as such at the time (roadmap history, PR #322): a trained algorithm absent from a test set's metadata got a reconciled all-false truth column scored as if it were real ground truth. MATLAB fixed #58 today (`c62408f`); this change ports the same fix. `PythiaEvaluateInput` gained a required `has_ground_truth` field (one entry per trained classifier, already computed by `InstanceSpace._build_test_algo_matrix` but previously discarded at its one call site); `PythiaStage.evaluate` now skips the confusion-matrix computation for a trained index with no ground truth, leaving its rates `NaN` and its confusion row zero - the same treatment a test-only algorithm with no trained-model slot already received. Verification: this repo's own report-only `matlab-oracle-verify.yml` job (added on PR #343) isolated exactly 8 fixture files whose content is attributable to real MATLAB code changes since the currently-pinned gold commit (as opposed to ~182 files of macOS-vs-Linux platform/run noise, confirmed via a control run pinned at the exact gold commit) - all 8 are PYTHIA's own `legacy_svm`/`trace3_pythia_skip` hyperparameter/selection/eval outputs, matching this exact code path. No current `tests/fixtures/matlab/current` parity test reads those 8 files for a `PythiaStage.evaluate` comparison (`test_current_matlab_trace_parity.py`'s similarly-named files live under a different `trace/` stage directory; `test_current_matlab_stages.py`'s one `PythiaEvaluateInput` call site is the `trace3_pythia_skip` variant, whose classifier slots are already all `None`), so this fix needed no fixture or pin change - confirmed by the full pytest suite passing unchanged (1047 passed) after the fix. Rewrote the one existing test that asserted the old fabricated-score behavior as the new skip behavior, and added a companion test proving the fix distinguishes "no test data" from "observed and genuinely bad everywhere" (both produce an all-false truth column, but only the former should be treated as missing data). |
| v1.77 | 2026-09-26 | **[Behavior-changing]** Refreshed `tests/fixtures/matlab/current` (the "verified v2" MATLAB oracle) from `98a01ac0513c0dd0f8a9bd91ed2926c871334d7b` (v0.9.1, macOS) to `fdad7a43a12a403c4b20e4e98bbf8a6e56538e2b` (current master at refresh time, Linux/GLNXA64) via `pyis_export_reference_data.m` run for real on `matlab-fixture-publish.yml` (a new one-off `workflow_dispatch` CI tool, not for routine use - the session's sandbox can reach github.com but not the Actions artifact blob-storage host, so it pushes the raw export directly to a branch by git rather than an artifact download). Root-caused with a control run (`matlab-oracle-verify.yml`'s `instancespace_ref` input, added same session): re-exporting at the *old* gold commit on this same Linux CI runner still showed 182 of 190 changed files, proving the bulk of the apparent drift was macOS-vs-Linux floating-point noise in SIFTED's GA and PILOT's iterative solver, not a MATLAB code change - only 8 files (all PYTHIA `legacy_svm`/`trace3_pythia_skip` outputs) were real, code-attributable drift, matching v1.76's #58 port. Generating on Linux going forward removes that noise category from future re-verifications. Updated `tools/fixture_provenance.py`'s `_GOLD_MATLAB_COMMIT` and `_VERIFIED_V2_CONTENT_ROOT_SHA256` (recomputed via the same sorted-path/sha256 digest `_manifest_content_root` uses); `_REFERENCE_V2_EXPORTER_SHA256` and `_CANONICAL_DATASET_SHA256` were unchanged (same exporter script, same input dataset) so were left alone. `tests/fixture_inventory.json` needed no regeneration - the file path set didn't change, only content within existing paths, and the inventory classifies by path. Fixing the tests against the new bundle needed more than a data swap: MATLAB's actual #58 fix (read from `andremun/InstanceSpace@master`'s current `core/PYTHIA.m`, not re-derived from the issue text alone) turned out broader than v1.76 first modeled - it also changed an **empty (untrained) classifier slot** from "zero accuracy, undefined precision/recall" to fully `NaN`, unifying that case with "no ground truth" rather than keeping it separate; `PythiaStage.evaluate`'s two internal loops and one test (`test_pythia_evaluate_empty_trained_slot_matches_matlab_skip`) were corrected to match. Also: one literal-commit test (`test_current_bundle_is_verified_r2026a_source`) updated to the new commit/platform; two literal-value dicts in `test_current_matlab_trace_parity.py` (`_BOUNDARY_AMBIGUITIES`, `_BOUNDARY_SUMMARY_VALUES`) emptied - the one boundary-ambiguous point each `trace3_default`/`trace3_pythia_skip` variant had under the old oracle no longer exists under the new one (real floating-point projection/footprint geometry shift, confirmed by the membership test's own actual-vs-expected diff going from 1 point to 0, not assumed). Verification: `poe check` and the full pytest suite (1047 passed, coverage 92.05%) both pass against the refreshed bundle. This PR is intentionally separate from #343 (docs) and #345 (the code fix alone) despite building on both, since bumping the gold fixture is its own, larger-blast-radius change that needs its own review. |
| v1.78 | 2026-09-26 | **Correction to v1.76/v1.77's "8 files are code-attributable" claim**, plus PR #346's Copilot review round. After v1.77 landed, running `matlab-oracle-verify.yml` twice more, independently, against the exact new gold commit (`fdad7a43a12a403c4b20e4e98bbf8a6e56538e2b`) reproduced the identical 182-file diff against the just-committed oracle both times - not random per-run noise, but a stable discrepancy that exposed a flaw in how the "8 code-attributable files" figure was isolated: it compared the new diff's count against the raw 190/182 totals, not against the *old* control run's own file list. Doing that comparison properly: ~175 of the 182 files are unstable at **both** the old and new gold commit (inherent SIFTED-GA/PILOT-solver-derived non-determinism, not narrowed by generating on Linux as v1.77 concluded); only **7 files** show genuinely new instability introduced between the two commits - all `pythia/legacy_svm` outputs (`hyperparameters`, `raw_metrics`, `selection0`, `selection1`, `summary`, `ysub`, plus explore's `eval_summary`) - plausibly tied to #58's changed scoring logic, but confirmed only at file-identity level (this sandbox's proxy blocks the Actions blob-storage host, so no value-level diff was possible). Separately, 6 files that *were* unstable under the old-commit control are now exact matches, consistent with the already-landed `pointwise_covers()` fix (#319), unrelated to #58. **Practical consequence:** a raw file-diff cannot safely gate the oracle refresh - the noise floor is too close to the 423-file total. Posted as correction comments on #344 and PR #346 rather than silently editing the v1.76/v1.77 rows above. Separately, Copilot's review of PR #346 found three Low findings, all fixed: `matlab-fixture-publish.yml` was referenced by `tests/matlab_export/README.md`'s provenance section but absent from the PR's branch (added it, dropping the self-guarded `push` trigger since GitHub had already registered the workflow from its first use on PR #343); the PR description's "1047 passed" was stale against the actual `pytest --collect-only` count of **1049** (two tests were added by the same branch's earlier per-instance-masking correction) - reconciled in the PR description. The `verify_against_real_matlab` CI check on PR #346's own head commit failed with the same 182-file diff - expected and non-blocking (job-level `continue-on-error: true`, by the workflow's own design, since it is the exporter's first run outside a human's local MATLAB and isn't wired to gate anything yet); documented on the PR rather than re-run, since the discrepancy is stable, not a flake. |
