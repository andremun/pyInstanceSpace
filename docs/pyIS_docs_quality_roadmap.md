...
| F8 | — | Unify `explore()` with build-time stage code (predict-mode dispatch on `PythiaStage`/`TraceStage`, matching MATLAB calling the same `PYTHIA()`/`TRACE()` in both modes) | **Scope corrected (v1.65) - "Narrowed by S1... PYTHIA half resolved... nothing left there to reconcile" was wrong.** Verified directly against current code: `_explore_pythia` (`instance_space.py:959`) still independently reimplements two pieces of `PythiaStage`'s build-time logic - the precision-weighted algorithm-selection formula (`PythiaStage._determine_selections`) and z-score normalisation (`PythiaStage._compute_znorm`). S1 only fixed *how the classifier itself is called* (native `predict`/`predict_proba`), not this separate duplication. Not started. | **[Behavior-changing risk]** — this refactors existing, working code (both TRACE and PYTHIA halves). The full `tests/matlab_reference/` validation suite must pass identically before/after; treat any tolerance-threshold change during this work as a red flag to investigate, not a "close enough" adjustment. |
| F9 | — | Expand `explore()` to full evaluation scope: algorithm reconciliation + ground-truth performance metrics, matching MATLAB's `evaluateTestSet` | **Scope corrected (v1.65).** Previously covered only PRELIM's ground-truth labels (`y_bin`/`y_best`/`p`/`beta`) - verified directly against MATLAB's `InstanceSpace.evaluateTestSet`/`PYTHIAevalMode` (`core/PYTHIA.m:305`) that "full evaluation scope" also means computing real `accuracy`/`precision`/`recall` against ground truth using the *already-trained* classifiers, which the existing pathway didn't cover at all. Deferred, transitively blocked on F8 (§6.0) - now a closer-to-real dependency than previously stated, since F9's metric computation and F8's PYTHIA-side de-duplication are two views of the same code (see companion pathways doc). | **[Additive]** — new fields default to `None`; existing feature-only callers see no change. Add explicit test coverage for the "no ground truth present" path specifically, to lock this in rather than assume it. |
...
| F8 (explore/build code reuse, TRACE **and PYTHIA** - corrected v1.65, was wrongly described as TRACE-only) | A deliberately-introduced bug in `TraceStage`'s footprint logic should break both the build-path and explore-path tests once they share code — proves the drift risk F8 is meant to close is actually closed. Same pattern for PYTHIA: break `_determine_selections`'s weighting formula, assert both `test_build_pythia.py` and an explore-path test fail together - S1 did not resolve this half as previously claimed here. |
