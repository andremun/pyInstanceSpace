...
### 6.3 External audit findings — PYTHIA/CLOISTER/SIFTED/PILOT/TRACE/PRELIM (added v1.43, updated v1.44-v1.45)

**Status as of v1.45: PRELIM, SIFTED, PILOT, PYTHIA, and CLOISTER verified and triaged —
confirmed findings either fixed and shipped, or explicitly deferred with a stated reason (never
silently dropped). TRACE is not yet verified** — its sub-issue (#302) is still in the original
"logged only, UNVERIFIED" state described below. A batch of 43 findings from an
