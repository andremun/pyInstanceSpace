...
**F12 — Guard implemented and verified (v1.34), performance rewrite deliberately deferred.**
`compute_uniformity()` now mirrors `core/FILTER.m`'s exact guard: fewer than 2 retained
...
all-coincident). The `O(n²)`→KD-tree rewrite is unchanged from "Not started" - picked as the
lower-risk half of this finding to land first, since the guard is a pure behaviour-improving
addition while the performance rewrite needs bit-identical-output verification before/after and
carries real risk of subtly changing `subset_index`/`is_dissimilar`/`is_visa` if not done
carefully.
...
### 6.3 External audit findings — PYTHIA/CLOISTER/SIFTED/PILOT/TRACE/PRELIM (added v1.43, updated v1.44)

**Status as of v1.44: PRELIM, SIFTED, PILOT, and PYTHIA verified and triaged — confirmed findings
either fixed and shipped, or explicitly deferred with a stated reason (never silently dropped).
CLOISTER and TRACE are not yet verified** — their sub-issues (#299, #302) are still in the
original "logged only, UNVERIFIED" state described below. A batch of 43 findings from an
...
- **PYTHIA (#298, 10 findings)** — verified; fixes shipped (see commit `b6508cb`). 7 fixed
  (issues 1, 2, 3, 5, 7, 8, 9); 3 deferred (issue 4 — deliberate prior F10/S1 design decisions,
  not bugs; issue 6 — sample weights in CV tuning, needs its own verification pass; issue 10 —
  eval/skip mode, overlaps F8/F9). Full breakdown in the GitHub comment on #298.
- **CLOISTER (#299, 5 findings)** — not yet verified.
- **TRACE (#302, 7 findings)** — not yet verified. Per direct instruction, when this verification
  happens, findings are to be documented (triaged, GitHub comment posted) but not implemented —
  same discipline as the stages above, stopping one step earlier.
