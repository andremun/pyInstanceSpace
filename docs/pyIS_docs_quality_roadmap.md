...
| v1.74 | 2026-09-25 | **[Additive]** Resolved #325 and #326 (docstring format and API-docs tooling). #326: the `format_docstrings` path had already moved from `matilda/` to `instancespace/` in `0a7999b`, but a check showed that `docconvert` reads only reST/epytext input. It made no change to the hybrid docstrings, with the old path or the new one. The task and the `docconvert` dev dependency are removed. #325: normalized the hybrid Google/numpy sections to numpy format in all affected modules (`Args` → `Parameters`, `name (type): text` → `name : type` plus an indented description, `Type: text` → `Type` plus an indented description, trailing-colon headers such as `Returns:` and the misspelled `Return`). Filled in the descriptions that ruff D417/D414 reported after the sections became recognizable, removed `_description_`/`TODO THIS` placeholders, and updated the package docstring's stage summary against Smith-Miles & Muñoz (2023). The new `poe docs` task (`pdoc instancespace -d numpy -o site`) is now part of `poe check`/`poe test`, `validation-tests.yml`, and `docs-pages.yml`, which switches from `-d google` back to `-d numpy`. pdoc exits non-zero on a docstring it cannot parse, so these builds stop the same drift from coming back. Verification: `pdoc -d numpy` builds every module; ruff, black and mypy --strict are clean; pytest: 1045 passed, 1 failed. The failure (`test_pythia_matches_legacy_matlab_snapshot`, one `pr0_hat` element in 2350 differs by 1.02e-13 against `atol=1e-13`) also occurs on the unmodified base commit and is not caused by this change. Docstrings and tooling only, no runtime code changes. |
